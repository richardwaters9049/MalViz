...
- **Worker queue source is split.** Next.js still writes BullMQ jobs while the Python worker claims PostgreSQL queued jobs for MVP reliability. Choose one authoritative queue source before production.
